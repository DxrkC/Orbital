...
namespace ns_menu
{
	...
	void add_key_bind(sol::this_state s, std::string name, std::vector <std::string> labels)
	{
		auto script = get_current_script(s);
		auto script_id = c_lua::get().get_script_id(script);

		auto& key_bind = c_lua::get().items.at(script_id);
		auto full_name = script + '.' + name;

		if (find_item(key_bind, full_name) != key_bind.end())
			return;
		key_bind.emplace_back(std::make_pair(full_name, menu_item(false)));
	}
...
void c_lua::refresh_scripts()
{
	...
	std::string folder = crypt_str("C:\\Orbital\\Scripts\\");
