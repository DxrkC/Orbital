...
void otheresp::holopanel(player_t* WhoUseThisBone, int hitbox_id, bool autodir)
{
	...
	if (g_cfg.misc.holo_panel)
	{
		...
		if (g_cfg.ragebot.anti_anti_defensive or g_cfg.ragebot.anti_defensive)
		{
		...
		render::get().text(fonts[NAME], angle.x + 110, angle.y - 65, Color(255, 255, 255, 255), 0, "Orbital [Free edition]");
	}
}