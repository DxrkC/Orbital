...
float antiaim::get_pitch(CUserCmd* m_pcmd)
{
	...
	case 3:
		...
		break;
	case 4:
		pitch = math::random_float( 85.0f, -85.0f);
		break;
	case 5:
		pitch = invert_jitter ? -85.0f : 85.0f;
		break;
	case 6:
		pitch = g_cfg.misc.anti_untrusted ? 85.0f : 535.0f;
		break;
	case 7:
		pitch = g_cfg.misc.anti_untrusted ? -85.0f : -535.0f;
		break;
	case 8:
		pitch = invert_jitter ? (g_cfg.misc.anti_untrusted ? 87.0f : 545.0f) : (g_cfg.misc.anti_untrusted ? -87.0f : -535.0f);
		break;
	}
...
float antiaim::get_yaw(CUserCmd* m_pcmd)
{
	...
	else
	{
		...
		case 2:
		{
			if (flip)
			{
				auto start_angle = (float)g_cfg.antiaim.type[type].range * 0.7f;
				auto end_angle = (float)g_cfg.antiaim.type[type].range * -0.7f;

				static auto angle = start_angle;

				auto angle_add_amount = (float)g_cfg.antiaim.type[type].speed * 0.7f;
...
bool antiaim::should_break_lby(CUserCmd* m_pcmd, int lby_type)
{
	...
	if (!g_ctx.globals.fakeducking && m_clientstate()->iChokedCommands > 15)
	{
	...
	if (animstate->m_velocity > 0.2f || fabs(animstate->flUpVelocity) > 101.0f)
		g_ctx.globals.next_lby_update = TICKS_TO_TIME(g_ctx.globals.fixed_tickbase + 16);
	else
	{
