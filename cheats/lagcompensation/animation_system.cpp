...
void lagcompensation::update_player_animations(player_t* e)
{
	...
	if (previous_record)
	{
		...
		if (ticks_chocked > 1)
		{
			...
			if (animlayers[4].m_flCycle < 0.6f && (!(e->m_fFlags() & FL_ONGROUND) || !(previous_record->flags & FL_ONGROUND)))
			{
...
#endif
	{
		...
		case RESOLVER_ZERO:
			...
			break;
		case RESOLVER_FIRST:
			animstate->m_flGoalFeetYaw = math::normalize_yaw(e->m_angEyeAngles().y + 59.0f);
			break;
		case RESOLVER_SECOND:
			animstate->m_flGoalFeetYaw = math::normalize_yaw(e->m_angEyeAngles().y - 59.0f);
			break;
		case RESOLVER_LOW_FIRST:
			animstate->m_flGoalFeetYaw = math::normalize_yaw(e->m_angEyeAngles().y + 31.0f);
			break;
		case RESOLVER_LOW_SECOND:
			animstate->m_flGoalFeetYaw = math::normalize_yaw(e->m_angEyeAngles().y - 31.0f);
			break;
