...
resolver_side resolver::FreeStand(player_t* e)
{
	...
	bool forward = fabsf(math::normalize_yaw(angle - GetForwardYaw(e))) < 80.f;

	auto misses = g_ctx.globals.missed_shots[e->EntIndex()];

	int S = DetectStand(e);

	bool HIGHDELTA = misses >= 1;
...
bool resolver::IsFakewalking(player_t* entity)
{
	...
	{
		if (player_record->layers[i].m_nSequence == 32 && player_record->layers[i].m_flWeight < 0.6f)
			stage1 = true;
		if (player_record->layers[i].m_nSequence == 8 && player_record->layers[i].m_flWeight > 0.002f)
			stage2 = true;
...
bool ValidPitch(player_t* entity)
{
	...
	return pitch == 0 || pitch > 80 || pitch < -80;
}
...
void resolver::resolve_yaw()
{
	...
	if (player->m_vecVelocity().Length2D() > 70)
	...
	if (animstate->m_velocity > 0.1f || fabs(animstate->flUpVelocity) > 130.f)
		valid_lby = animstate->m_flTimeSinceStartedMoving < 0.25f;
	...
	if (player->m_vecVelocity().Length2D() > 245 || !player->m_fFlags() & FL_ONGROUND)
		player_record->side = RESOLVER_ORIGINAL;
	...
	else if (fabs(delta) > 35.0f && valid_lby)
	{
	...
	else
	{
		...
		if (m_globals()->m_curtime - lock_side > 1.0f)
		{
			...
			else
			{
				...
				if (fabs(first_position - second_position) > 2.0f)
