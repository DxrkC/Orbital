...
void misc::watermark()
{
	...
	{
		if (BETA)
			name_cheat = crypt_str("Orbital.cc [Beta]");
		else
			name_cheat = crypt_str("Orbital.cc");
	}
	else
	...
	render::get().rect_filled(width - 10 - box_width, 10, box_width, 2, Color(167, 192, 63,245));
	render::get().rect_filled(width - 10 - box_width, 12, box_width, 18, Color(32, 32, 32,255));

	render::get().text(fonts[KEYBINDSLIST], width - 10 - box_width + 5, 21, Color(245, 225, 253, 222), HFONT_CENTERED_Y, watermark.c_str());
}
