...
void eventlogs::add(std::string text, bool full_display)
{
...
#if RELEASE
#if BETA
		m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("[ Orbital.cc ] ")); 
#else
		m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("[ Orbital.cc ] "));
#endif
#else
		m_cvar()->ConsoleColorPrintf(Color(147, 190, 66,255), crypt_str("[ Orbital.cc ] "));
#endif
	...
	if (g_cfg.misc.log_output[EVENTLOG_OUTPUT_CHAT])
	{
		...
		auto log = crypt_str("[ \x0C Orbital.cc \x01] ") + text;
