				...
				if (!current_shot->hurt_player)
				{
					...
					else if (g_cfg.misc.events_to_log[EVENTLOG_HIT])
					{
						...
						if (current_shot->occlusion)
							m_cvar()->ConsoleColorPrintf(Color(255, 255, 0), crypt_str("[Orbital.cc] Missed shot due to occlusion. [Resolver Math]\n"));
						else if (current_shot->shot_info.hitchance == 100)
							m_cvar()->ConsoleColorPrintf(Color(255, 0, 0), crypt_str("[Orbital.cc] Missed shot due to prediction.\n"));
						else
							m_cvar()->ConsoleColorPrintf(Color(255, 200, 0), crypt_str("[Orbital.cc] Missed shot due to spread. [RageBot]\n"));
					}
				}
			}
